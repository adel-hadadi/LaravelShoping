                        ...
                        <tbody>
                            @foreach ($posts as $key => $post)
                                <tr>
                                    ...
                                    <td><img src="{{ asset($post->image['indexArray'][$post->image['currentImage']]) }}"
                                            alt="" class="max-height-3rem rounded" height="40"></td>
                                    <td>
                                        <label>
                                    ...
                                    <td>
                                        <label>
                                            <input id="{{ $post->id }}-commentable" onchange="changeCommentable({{ $post->id }})"
                            ...
                            <td class="width-16-rem text-left">
                                <a href="{{ route('admin.content.post.edit', $post->id) }}"
                                    ...
                                    <form action="{{ route('admin.content.post.destroy', $post->id) }}" class="d-inline"> 
                                        @csrf
                                        @method('delete')
                ...
                success: function(response) {
                    if (response.status) {
                        if (response.checked) {
                            ...
                            successToast('پست با موفقیت فعال شدی')
                        } else {
